...
function CompraInsumos(props) {
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            ...
            unidad: Yup.string()                    
                .required('Debe ingresar la unidad del insumo'), 
            descripcion: Yup.string()                    
                .max(100,"Debe tener menos de 100 caracteres"),
            valor: Yup.number()                    
                .required('Debe ingresar el valor del insumo')
    ...
    return (        
        ...
        <Toaster/>          
                    <div className="w-100% text-center">
                        <h2 className="font-medium leading-tight text-4xl mt-0 mb-2 text-amber-600">Compra de insumos</h2>
                    </div>
        <form onSubmit={formik.handleSubmit}>
            ...
            <input type="submit" value="Añadir compra de insumos" className="bg-amber-600 hover:bg-amber-800 transition-colors cursos-pointer uppercase font-bold w-full p-3 text-white rounded-lg" />    
        </form>                                        
                </div>
