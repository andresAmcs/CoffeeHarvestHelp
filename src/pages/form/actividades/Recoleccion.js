...
function Recoleccion(props) {
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            ...
            lote: Yup.string()    
                .required("Debe de ingresar el lote en el que recolecto"),
            kilos: Yup.number()    
                .required("Debe de ingresar los kilos recolectados"),
            kilosAproxSeco: Yup.number()    
                .required("Debe de ingresar los kilos aproximados"),
    ...
    return (        
        ...
        <Toaster/>          
                    <div className="w-100% text-center">
                        <h2 className="font-medium leading-tight text-4xl mt-0 mb-2 text-amber-600">Recolección</h2>
                    </div>
        <form onSubmit={formik.handleSubmit}>
            ...
            <div className="my-5 mx-auto w-100% ">
                <label className="w-[60%] block uppercase text-gray-800 font-bold" htmlFor="kilos">
                    Kilos Recolectados
                </label>
                <div className="flex">
                    <span className="inline-flex items-center px-3 text-sm text-gray-900 bg-gray-200 border border-r-0 border-gray-300 rounded-l-md dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600">
                ...
                {formik.touched.kilos && formik.errors.kilos ? (    
                    <div className='text-red-600 text-lg font-bold'>{formik.errors.kilos}</div>    
                    ) : null}
            </div>

            <div className="my-5 mx-auto w-100% ">
                <label className="w-[60%] block uppercase text-gray-800 font-bold" htmlFor="kilosAproxSeco">
                    Kilos Aproximado Seco 
                </label>
                <div className="flex">
                    <span className="inline-flex items-center px-3 text-sm text-gray-900 bg-gray-200 border border-r-0 border-gray-300 rounded-l-md dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600">
                        <i className={`fa-solid fa-home`}></i>
                    ...
                    <input
                        ...
                        onChange={formik.handleChange}
                        onBlur={formik.handleBlur}
                        value={formik.values.kilosAproxSeco}
                        className="rounded-none rounded-r-lg bg-gray-50 border text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5"
                    />
    
                </div>
                {formik.touched.kilosAproxSeco && formik.errors.kilosAproxSeco ? (    
                    <div className='text-red-600 text-lg font-bold'>{formik.errors.kilosAproxSeco}</div>    
                    ) : null}
            </div>
